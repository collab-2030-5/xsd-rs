...
impl OadrResponse {
        ...
        W: std::io::Write,
    {
        ...
        let start = if write_type {
            start.attr("xsi:type", "oadrResponse")
        } else {
            start
        };
        writer.write(start)?;
...
impl crate::xsd_util::WriteXml for OadrResponse {
    fn write<W>(
        &self,
...
impl OadrResponse {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "oadrResponse")?;
        OadrResponse::read(reader, &attr, "oadrResponse")
    }
}

impl crate::xsd_util::ReadXml for OadrResponse {
        ...
        R: std::io::Read,
    {
        ...
        match OadrResponse::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
