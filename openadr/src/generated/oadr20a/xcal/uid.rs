...
impl Uid {
    ...
    where
        W: std::io::Write,
    {
        crate::xsd_util::write_simple_element(writer, "xcal:text", self.xcal_text.as_str())?;
        Ok(())
    }
...
impl crate::xsd_util::WriteXml for Uid {
    fn write<W>(
        &self,
        ...
        W: std::io::Write,
    {
        ...
        self.write_with_name(&mut writer, "xcal:uid", true, false)?;
        Ok(())
    }
}
...
impl Uid {
        ...
        R: std::io::Read,
    {
        ...
        let mut xcal_text: crate::xsd_util::SetOnce<String> = Default::default();
            ...
            match reader.next()? {
                ...
                xml::reader::XmlEvent::StartElement { name, .. } => {
                    match name.local_name.as_str() {
                        "text" => xcal_text.set(crate::xsd_util::read_string(reader, "text")?)?,
                        name => {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "uid")?;
        Uid::read(reader, &attr, "uid")
    }
}

impl crate::xsd_util::ReadXml for Uid {
        ...
        R: std::io::Read,
    {
        ...
        match Uid::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
