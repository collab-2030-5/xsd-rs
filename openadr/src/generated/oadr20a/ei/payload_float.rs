...
impl PayloadFloat {
    ...
    where
        W: std::io::Write,
    {
        crate::xsd_util::write_element_using_to_string(writer, "ei:value", self.value)?;
        Ok(())
    }
        ...
        W: std::io::Write,
    {
        ...
        let start = if write_type {
            start.attr("xsi:type", "payloadFloat")
        } else {
            start
        };
        writer.write(start)?;
...
impl crate::xsd_util::WriteXml for PayloadFloat {
    fn write<W>(
        &self,
...
impl PayloadFloat {
        ...
        R: std::io::Read,
    {
        ...
        let mut value: crate::xsd_util::SetOnce<f32> = Default::default();
            ...
            match reader.next()? {
                ...
                xml::reader::XmlEvent::StartElement { name, .. } => {
                    match name.local_name.as_str() {
                        "value" => {
                            value.set(crate::xsd_util::read_type_from_string(reader, "value")?)?
                        }
                        name => {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "payloadFloat")?;
        PayloadFloat::read(reader, &attr, "payloadFloat")
    }
}

impl crate::xsd_util::ReadXml for PayloadFloat {
        ...
        R: std::io::Read,
    {
        ...
        match PayloadFloat::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
