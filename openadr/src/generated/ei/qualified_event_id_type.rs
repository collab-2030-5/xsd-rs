...
impl QualifiedEventIdType {
    fn write_elem<W>(&self, writer: &mut EventWriter<W>) -> core::result::Result<(), xml::writer::Error> where W: std::io::Write {
        ...
        xsd_util::write_element_using_to_string(writer, "ei:modificationNumber", self.ei_modification_number)?;
        Ok(())
    }
...
impl xsd_api::WriteXml for QualifiedEventIdType {
                ...
                xml::reader::XmlEvent::StartElement { name, .. } => {
                    match name.local_name.as_str() {
                        ...
                        "ei:modificationNumber" => {
                            ei_modification_number.set(xsd_util::read_type_from_string(reader, "ei:modificationNumber")?)?
                        }
...
impl xsd_api::ReadXml for QualifiedEventIdType {
    fn read<R>(r: &mut R) -> core::result::Result<Self, xsd_api::ErrorWithLocation> where R: std::io::Read {
        let mut reader = xml::reader::EventReader::new(r);

        match QualifiedEventIdType::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
