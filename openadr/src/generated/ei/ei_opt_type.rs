...
impl EiOptType {
    fn write_elem<W>(&self, writer: &mut EventWriter<W>) -> core::result::Result<(), xml::writer::Error> where W: std::io::Write {
        ...
        xsd_util::write_simple_element(writer, "ei:createdDateTime", self.ei_created_date_time.as_str())?;
        Ok(())
    }
    ...
    pub(crate) fn write_with_name<W>(&self, writer: &mut EventWriter<W>, name: &str, top_level: bool, write_type: bool) -> core::result::Result<(), xml::writer::Error> where W: std::io::Write {
        ...
        let start = match &self.ei_schema_version {
            Some(attr) => {
                start.attr("ei:schemaVersion", attr.as_str())
            },
            None => start,
        };
        // ---- end attributes ----
...
impl EiOptType {
    pub(crate) fn read<R>(reader: &mut xml::reader::EventReader<R>, attrs: &Vec<xml::attribute::OwnedAttribute>, parent_tag: &str) -> core::result::Result<Self, xsd_api::ReadError> where R: std::io::Read {
        ...
        let mut xcal_vavailability : xsd_util::SetOnce<crate::xcal::VavailabilityType> = Default::default();
            ...
            match reader.next()? {
                ...
                xml::reader::XmlEvent::StartElement { name, attributes, .. } => {
                    match name.local_name.as_str() {
                        "ei:optID" => {
                            ei_opt_id.set(xsd_util::read_string(reader, "ei:optID")?)?
                        }
                        "ei:optType" => {
                        ...
                        "emix:marketContext" => {
                            emix_market_context.set(xsd_util::read_string(reader, "emix:marketContext")?)?
                        }
                        "ei:venID" => {
                            ei_ven_id.set(xsd_util::read_string(reader, "ei:venID")?)?
                        }
                        "xcal:vavailability" => {
                            xcal_vavailability.set(crate::xcal::VavailabilityType::read(reader, &attributes, "xcal:vavailability")?)?
                        }
                        "ei:createdDateTime" => {
                            ei_created_date_time.set(xsd_util::read_string(reader, "ei:createdDateTime")?)?
                        }
