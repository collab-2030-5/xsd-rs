...
impl ReportPayloadType {
    fn write_elem<W>(&self, writer: &mut EventWriter<W>) -> core::result::Result<(), xml::writer::Error> where W: std::io::Write {
        ...
        self.ei_payload_base.write_with_name(writer, "ei:payloadBase")?;
        Ok(())
    }
            ...
            match reader.next()? {
                ...
                xml::reader::XmlEvent::StartElement { name, attributes, .. } => {
                    match name.local_name.as_str() {
                        "ei:rID" => {
                            ei_r_id.set(xsd_util::read_string(reader, "ei:rID")?)?
                        }
                        "ei:confidence" => {
                            ei_confidence.set(xsd_util::read_type_from_string(reader, "ei:confidence")?)?
                        }
                        "ei:accuracy" => {
                            ei_accuracy.set(xsd_util::read_type_from_string(reader, "ei:accuracy")?)?
                        }
                        "ei:payloadBase" => {
                            ei_payload_base.set(base::PayloadBaseType::read(reader, &attributes, "ei:payloadBase")?)?
                        }
