...
impl EnergyApparentType {
    fn write_elem<W>(&self, writer: &mut EventWriter<W>) -> core::result::Result<(), xml::writer::Error> where W: std::io::Write {
        ...
        write_simple_tag(writer, "scale:siScaleCode", self.scale_si_scale_code.as_str())?;
        write_simple_tag(writer, "itemDescription", self.item_description.as_str())?;
        write_simple_tag(writer, "itemUnits", self.item_units.as_str())?;
        write_simple_tag(writer, "scale:siScaleCode", self.scale_si_scale_code.as_str())?;
        Ok(())
...
impl EnergyApparentType {
    pub(crate) fn read<R>(reader: &mut xml::reader::EventReader<R>, attrs: &Vec<xml::attribute::OwnedAttribute>, parent_tag: &str) -> core::result::Result<Self, ReadError> where R: std::io::Read {
        ...
        let mut scale_si_scale_code : SetOnce<enums::SiScaleCodeType> = Default::default();
        let mut item_description : SetOnce<String> = Default::default();
        let mut item_units : SetOnce<String> = Default::default();
        let mut scale_si_scale_code : SetOnce<enums::SiScaleCodeType> = Default::default();
            ...
            match reader.next()? {
                ...
                xml::reader::XmlEvent::StartElement { name, .. } => {
                    match name.local_name.as_str() {
                        ...
                        "itemUnits" => {
                            item_units.set(read_string(reader, "itemUnits")?)?
                        }
                        "scale:siScaleCode" => {
