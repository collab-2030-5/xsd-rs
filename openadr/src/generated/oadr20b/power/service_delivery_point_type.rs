...
impl ServiceDeliveryPointType {
    ...
    where
        W: std::io::Write,
    {
        crate::xsd_util::write_simple_element(writer, "power:node", self.power_node.as_str())?;
        Ok(())
    }
        ...
        W: std::io::Write,
    {
        ...
        let start = if write_type {
            start.attr("xsi:type", "ServiceDeliveryPointType")
        } else {
            start
        };
        writer.write(start)?;
...
impl crate::xsd_util::WriteXml for ServiceDeliveryPointType {
    fn write<W>(
        &self,
...
impl ServiceDeliveryPointType {
        ...
        R: std::io::Read,
    {
        ...
        let mut power_node: crate::xsd_util::SetOnce<String> = Default::default();
            ...
            match reader.next()? {
                ...
                xml::reader::XmlEvent::StartElement { name, .. } => {
                    match name.local_name.as_str() {
                        "node" => power_node.set(crate::xsd_util::read_string(reader, "node")?)?,
                        name => {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "serviceDeliveryPoint")?;
        ServiceDeliveryPointType::read(reader, &attr, "serviceDeliveryPoint")
    }
}

impl crate::xsd_util::ReadXml for ServiceDeliveryPointType {
        ...
        R: std::io::Read,
    {
        ...
        match ServiceDeliveryPointType::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
