...
impl BaseUnitType {
        ...
        W: std::io::Write,
    {
        ...
        let start = if write_type {
            start.attr("xsi:type", "BaseUnitType")
        } else {
            start
        };
        writer.write(start)?;
...
impl crate::xsd_util::WriteXml for BaseUnitType {
    fn write<W>(
        &self,
...
impl BaseUnitType {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "customUnit")?;
        BaseUnitType::read(reader, &attr, "customUnit")
    }
}

impl crate::xsd_util::ReadXml for BaseUnitType {
        ...
        R: std::io::Read,
    {
        ...
        match BaseUnitType::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
