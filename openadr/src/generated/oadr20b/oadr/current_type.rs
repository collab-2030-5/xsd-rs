...
impl CurrentType {
        ...
        W: std::io::Write,
    {
        ...
        let start = if write_type {
            start.attr("xsi:type", "CurrentType")
        } else {
            start
        };
        writer.write(start)?;
...
impl crate::xsd_util::WriteXml for CurrentType {
    fn write<W>(
        &self,
...
impl CurrentType {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "current")?;
        CurrentType::read(reader, &attr, "current")
    }
}

impl crate::xsd_util::ReadXml for CurrentType {
        ...
        R: std::io::Read,
    {
        ...
        match CurrentType::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
