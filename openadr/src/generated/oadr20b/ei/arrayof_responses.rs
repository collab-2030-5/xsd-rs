...
impl crate::xsd_util::WriteXml for ArrayofResponses {
    fn write<W>(
        &self,
...
impl ArrayofResponses {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "responses")?;
        ArrayofResponses::read(reader, &attr, "responses")
    }
}

impl crate::xsd_util::ReadXml for ArrayofResponses {
        ...
        R: std::io::Read,
    {
        ...
        match ArrayofResponses::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
