...
impl crate::xsd_util::WriteXml for AvailableType {
    fn write<W>(
        &self,
...
impl AvailableType {
    ...
    where
        R: std::io::Read,
    {
        let attr = crate::xsd_util::read_start_tag(reader, "available")?;
        AvailableType::read(reader, &attr, "available")
    }
}

impl crate::xsd_util::ReadXml for AvailableType {
        ...
        R: std::io::Read,
    {
        ...
        match AvailableType::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
