...
impl FrequencyType {
        ...
        W: std::io::Write,
    {
        ...
        let start = if write_type {
            start.attr("xsi:type", "oadr:FrequencyType")
        } else {
            start
        };
        writer.write(start)?;
...
impl xsd_api::WriteXml for FrequencyType {
    fn write<W>(
        &self,
        ...
        W: std::io::Write,
    {
        ...
        self.write_with_name(&mut writer, "oadr:FrequencyType", true, false)?;
        Ok(())
    }
}
...
impl FrequencyType {
    ...
    where
        R: std::io::Read,
    {
        let attr = xsd_util::read_start_tag(reader, "FrequencyType")?;
        FrequencyType::read(reader, &attr, "oadr:FrequencyType")
    }
}

impl xsd_api::ReadXml for FrequencyType {
        ...
        R: std::io::Read,
    {
        ...
        match FrequencyType::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
