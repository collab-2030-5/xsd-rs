...
impl OadrEventType {
        ...
        W: std::io::Write,
    {
        ...
        let start = if write_type {
            start.attr("xsi:type", "oadrEventType")
        } else {
            start
        };
        writer.write(start)?;
...
impl xsd_api::WriteXml for OadrEventType {
    fn write<W>(
        &self,
        ...
        W: std::io::Write,
    {
        ...
        self.write_with_name(&mut writer, "oadr:oadrEventType", true, false)?;
        Ok(())
    }
}
...
impl OadrEventType {
    ...
    where
        R: std::io::Read,
    {
        let attr = xsd_util::read_start_tag(reader, "oadrEventType")?;
        OadrEventType::read(reader, &attr, "oadrEventType")
    }
}

impl xsd_api::ReadXml for OadrEventType {
        ...
        R: std::io::Read,
    {
        ...
        match OadrEventType::read_top_level(&mut reader) {
            Ok(x) => Ok(x),
            Err(err) => {
