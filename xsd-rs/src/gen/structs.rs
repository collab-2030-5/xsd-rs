...
fn write_deserializer_impl(w: &mut dyn Write, st: &Struct) -> std::io::Result<()> {
    ...
    writeln!(w, "impl {} {{", st.id.name.to_upper_camel_case())?;
    indent(w, |w| {
        writeln!(w, "pub(crate) fn read<R>(reader: &mut xml::reader::EventReader<R>, attrs: &[xml::attribute::OwnedAttribute], parent_tag: &str) -> core::result::Result<Self, xsd_api::ReadError> where R: std::io::Read {{")?;
        indent(w, |w| {
